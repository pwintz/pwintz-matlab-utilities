function result = mapColumns(map_function, input_arrays)
  ...
  for col = 1:n_cols
    ...
    %   warning("Output of the map_function handle\n\n%s\nis not a %s instead of a double, string, or char. It might not be convertable to a matrix.", formattedDisplayText(map_function), class(out_cell{col}))
    % end % End of if block.
  ...
  pwintz.assertions.assertNumRows(result, n_cols, "The output of mapColumns must have the same number of rows as the inputs. Output size " + mat2str(size(result)));
